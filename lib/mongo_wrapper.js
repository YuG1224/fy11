 ...
 * @param {Object} sort sort オプション
 * @param {Function} callback コールバックファンクション
 * @param {Boolean} array true の場合は配列を返す、false の場合は cursor を返す
 */
MongoWrapper.prototype.find = function (collectionName, criteria, fields, options, sort, callback, array) {
  var self = this;
  self.getCollection(collectionName, function (collection) {
	  if (array) {
	    collection.find(criteria, fields, options).sort(sort).toArray(function (err, doc) {
		    self.errFunc(err);
		    callback(err, doc);
	    });
	  } else {
